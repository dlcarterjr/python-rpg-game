    ...
    def attack(self, enemy):
        ...
        if self.grenade:  ## Checks for holy handgrenade.
            ...
            if throw_grenade.lower() == 'y':
                enemy.health = 0  ## Sets enemy health to 0 (dead).
                self.grenade = False ## Resets attribute to false (only use grenade once.)
                return
...
class Battle(object):
    def do_battle(self, hero, enemy):
        ...
        if hero.alive():
            ...
            print(f"You collect a bounty of {enemy.bounty} coins.")  ## Adds enemy bounty for each character to hero's coins.
...
class SuperTonic(object):  ## Added SuperTonic to store.
    ...
    name = 'super tonic'
    def apply(self, character):
        character.health += 10  ## Adds 10 points to hero health.
        print("%s's health increased to %d." % (character.name, character.health))
...
class Evade(object):  ## Added evade to store. See line 61 for evade success calculation.
    cost = 15
    name = 'evade'
    def apply(self, hero):
        if hero.evade >= 38:  ## Check if evade ability is maxed out at 50% probability.
